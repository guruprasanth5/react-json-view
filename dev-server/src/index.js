...
class App extends React.Component {
    constructor(props){
        ...
        this.onUnMask = this.onUnMask.bind(this)
    }
    onMask(value){
        let cloneMaskData = JSON.parse(JSON.stringify(this.state.maskData))
        cloneMaskData.push(value)
        this.setState({
            maskData: cloneMaskData
        })
    }
    onUnMask(value){
        let cloneMaskData = JSON.parse(JSON.stringify(this.state.maskData))
        let updatedData = cloneMaskData.filter(item=>{
            return item.toString() !== value.toString()
        })

        this.setState({
            maskData:updatedData
        })
    }
    render() {
        return(
